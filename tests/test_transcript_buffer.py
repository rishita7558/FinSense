...
def test_transcript_buffer_merges_partial_and_final_segments():
    ...
    buffer.upsert(
        TranscriptSegment(
            session_id="s1", segment_id="1", sequence=1, text="I think we should", is_final=False
        )
    )
    buffer.upsert(
        TranscriptSegment(
            session_id="s1", segment_id="1", sequence=1, text="I think we should", is_final=True
        )
    )
    buffer.upsert(
        TranscriptSegment(
            session_id="s1", segment_id="2", sequence=2, text="start a SIP", is_final=True
        )
    )
