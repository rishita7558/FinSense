...
def main():
    ...
    run_service("FastAPI Backend", f'"{venv_python}" -m uvicorn backend.app:app --host 127.0.0.1 --port 8000', "logs/backend.log")
    time.sleep(15) # Give Backend time to load heavy AI models into memory

    # 3. Start Streamlit Frontend
    print("[FinSense] Starting Streamlit Frontend...")
    p_frontend = subprocess.Popen(
        f'"{venv_python}" -m streamlit run frontend/dashboard.py',
        shell=True
    )
